...
type Columns<S extends Schema> = {
	[K in keyof S as `${typeof COLUMN_PREFIX}${KeyToColumnName<
		string & K
	>}`]: number;
};

type TempColumnsForUnique<S extends Schema> = {
	[K in keyof S as `${typeof UNIQUE_TEMP_COLUMN_PREFIX}${KeyToColumnName<
		string & K
...
export default abstract class Chart<T extends Schema> {
	...
	private readonly repositoryForHour: Repository<{
		id: number;
		group?: string | null;
		date: number;
	}>;
	private readonly repositoryForDay: Repository<{
		id: number;
		group?: string | null;
		date: number;
	}>;
		...
		const update = async (
			...
			for (const [k, v] of Object.entries(finalDiffs)) {
				if (typeof v === 'number') {
					...
					if (v < 0)
						queryForHour[name] = (): string => `"${name}" - ${Math.abs(v)}`;
					if (v > 0) queryForDay[name] = (): string => `"${name}" + ${v}`;
					if (v < 0)
						queryForDay[name] = (): string => `"${name}" - ${Math.abs(v)}`;
					...
					if (itemsForHour.length > 0)
						queryForHour[tempColumnName] = (): string =>
							`array_cat("${tempColumnName}", '{${itemsForHour.join(
								',',
							)}}'::varchar[])`;
					if (itemsForDay.length > 0)
						queryForDay[tempColumnName] = (): string =>
