...
import fetch, { File, RequestInit, Response } from 'node-fetch';
...
export const api = async <
	T extends string = keyof misskey.Endpoints,
	U extends Record<string, unknown> = T extends keyof misskey.Endpoints
		? misskey.Endpoints[T]['req']
...
export const failedApiCall = async (
	...
	const body = z
		.object({ error: z.object({ code: z.string(), id: z.string() }) })
		.parse(res.body);

	assert.strictEqual(body.error.code, code, inspect(res.body));
	assert.strictEqual(body.error.id, id, inspect(res.body));
...
const request = async (
	...
	const res = await relativeFetch(path, {
		method: 'POST',
		headers,
		body: JSON.stringify({ ...bodyAuth, ...params }),
		redirect: 'manual',
	});
...
const relativeFetch = async (
	path: string,
	init?: RequestInit | undefined,
): Promise<Response> => {
	return await fetch(
		new URL(path, `http://127.0.0.1:${config.port}/`).toString(),
		init,
	);
};

export const signup = async (params?: Partial<unknown>): Promise<unknown> => {
...
export const post = async (
	...
	const hoge = z
		.object({ body: z.object({ createdNote: z.unknown() }) })
		.parse(res);
	return hoge.body.createdNote;
};
...
export const react = async (
	...
	await api(
		...
		{ noteId: note.id, reaction: reaction },
		user,
	);
};
...
export const userList = async (
	...
	const res = await api('users/lists/create', { name: 'test' }, user);
	return res.body;
};
			...
			policies: {
				...fromEntries(
					toEntries(DEFAULT_POLICIES).map(([k, v]) => [
						k,
						{ priority: 0, useDefault: true, value: v },
					]),
				),
				...policies,
			},
...
export const uploadUrl = async (
	...
	const ws = await connectStream(user, 'main', (msg_) => {
		const msg = z
			.object({ type: z.unknown(), body: z.record(z.string(), z.unknown()) })
			.parse(msg_);
	...
	await api('drive/files/upload-from-url', { url, marker, force: true }, user);
...
export function connectStream(
	user: UserToken,
	channel: string,
	listener: (message: Record<string, unknown>) => unknown,
	params?: unknown,
): Promise<WebSocket> {
	return new Promise((resolve, reject) => {
		...
		ws.on('unexpected-response', (_req, res) => reject(res));
		ws.on('open', () => {
			ws.on('message', (data) => {
				const msg = z
					.object({
...
export const waitFire = async (
	...
	channel: string,
	trgr: () => unknown,
	cond: (msg: Record<string, unknown>) => boolean,
	params?: unknown,
): Promise<unknown> => {
...
>(
	...
	const rangeToParam = (p: {
		limit?: number | undefined;
		until?: Entity | undefined;
		since?: Entity | undefined;
	}): object => {
		if (offsetBy === 'id') {
	...
	for (const limit of [1, 5, 10, 100, undefined]) {
		...
		if (ordering === 'desc') {
			...
			while (last.length !== 0) {
				actual.push(...last);
				last = await fetchEntities(
					rangeToParam({ limit, until: last.at(-1), since: end }),
				);
			}
			actual.push(end);
